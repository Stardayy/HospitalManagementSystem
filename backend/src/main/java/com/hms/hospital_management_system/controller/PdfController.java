...
public class PdfController {
        ...
        public ResponseEntity<byte[]> generateAdmissionReport(@PathVariable Long admissionId) {
                ...
                if (admission.getPatient() != null) {
                        vitalSigns = vitalSignsRepository
                                        .findByPatientIdOrderByRecordedAtDesc(admission.getPatient().getId());
                }

                List<MedicalRecord> records = List.of();
                if (admission.getPatient() != null) {
                        records = medicalRecordRepository
                                        .findByPatientIdOrderByRecordDateDesc(admission.getPatient().getId());
                }
        ...
        @PreAuthorize("hasAnyRole('ADMIN', 'DOCTOR', 'PHARMACIST') or @patientSecurityService.isPrescriptionOwner(#prescriptionId, principal)")
        public ResponseEntity<byte[]> generatePrescriptionReport(@PathVariable Long prescriptionId) {
                byte[] pdfContent = pdfService.generatePrescriptionReport(prescriptionId);

                String filename = "prescription_" + prescriptionId + "_" +
                                LocalDate.now().format(DateTimeFormatter.ofPattern("yyyyMMdd")) + ".pdf";
        ...
        @PreAuthorize("hasAnyRole('ADMIN', 'DOCTOR') or @patientSecurityService.isMedicalRecordOwner(#recordId, principal)")
        public ResponseEntity<byte[]> generateMedicalRecordReport(@PathVariable Long recordId) {
                byte[] pdfContent = pdfService.generateMedicalRecordReport(recordId);

                String filename = "medical_record_" + recordId + "_" +
                                LocalDate.now().format(DateTimeFormatter.ofPattern("yyyyMMdd")) + ".pdf";
